...
function loadFile(file, container) {
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(file, 'utf8', (error, data) => {
      ...
      // syntax highlight
      const html = new highlights().highlightSync({
        fileContents: data,
        scopeName: 'source.js'
      });
...
function drawBridge(changeLeft, changeRight, lineLeft, lineRight, leftOffset, rightOffset, bridge) {
  ...
  const action = changeLeft.removed && changeRight.added ? 'edit' : (changeLeft.removed ? 'delete' : 'add');
  const leftHeight = changeLeft.removed ? (changeLeft.count * lineHeight) : 0;
  const rightHeight = changeRight.added ? (changeRight.count * lineHeight) : 0;
...
$(() => {
  ...
  $(window).on('scroll', (e) => {
    ...
    let align = alignByLine[focalLine],
    ...
    const percent = ((focalPoint / lineHeight) - align.gutter.first) / align.gutter.size;
